 ...
 * exceptions out of event handlers and it's not always feasible to handle exceptions within the handlers themselves.
...
public class SqlRoseException extends RuntimeException {
    ...
    public SqlRoseException(ErrorCode code) {
        super(code == null ? null : code.getValue());
        this.code = code;
    }

    public SqlRoseException(ErrorCode code, Throwable cause) {
        super(code == null ? null : code.getValue(), cause);
        this.code = code;
    }
     ...
     * that might be used when resolving the {@link #getCode() code} to a localized message and formatting the latter
     */
